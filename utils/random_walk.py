...
class Graph_RandomWalk():
    ...
    def node2vec_walk(self, walk_length, start_node):#传入游走长度以及开始节点
        '''
        ...
        while len(walk) < walk_length:  # 游走长度20
            cur = walk[-1]#取walk列表的最近一个节点即随机游走的最近一点
            cur_nbrs = sorted(G.neighbors(cur))  # 按照节点的邻居节点排序进行概率选择;
            if len(cur_nbrs) > 0:
                if len(walk) == 1:  # 初始节点
                    walk.append(cur_nbrs[alias_draw(alias_nodes[cur][0], alias_nodes[cur][1])])  # alias: 采样，概率 alias_draw传入alias的两个分配矩阵,返回采样的节点索引;cur_nbrs最终获取了采样的邻居节点
                else:
                    prev = walk[-2]
                    next = cur_nbrs[alias_draw(alias_edges[(prev, cur)][0],  # node2vec
                        alias_edges[(prev, cur)][1])]#选择下一节点
                    walk.append(next)
            else:
                break
        return walk
    ...
    def simulate_walks(self, num_walks, walk_length):
        '''
        Repeatedly simulate random walks from each node.重复模拟每个节点的随机游走
        '''
        G = self.G
        walks = []
        nodes = list(G.nodes())#获取G中的节点序列
        for walk_iter in range(num_walks):  # 每个节点遍历的次数
            random.shuffle(nodes)#打乱顺序
            for node in nodes:#以每一节点为Start_node
                walks.append(self.node2vec_walk(walk_length=walk_length, start_node=node))

        return walks#返回从每一节点出发多的随机游走的结果，长度为Walk_lenhth
    ...
    def get_alias_edge(self, src, dst):
        ...
        return alias_setup(normalized_probs)#返回alias的两个矩阵
...
def alias_setup(probs):#设计alis分配矩阵  两个
    '''
...
def alias_draw(J, q):#使用alias进行采样
    '''
