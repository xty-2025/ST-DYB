...
def fixed_unigram_candidate_sampler(true_clasees,
    ...
    for i in range(true_clasees.shape[0]):
        ...
        for tabo in sorted(taboo, reverse=True):
            ...
            dist.pop(tabo)
        sample = np.random.choice(candidate[user_num:len(candidate)], size=num_sampled, replace=unique, p=dist[user_num:len(dist)]/np.sum(dist[user_num:len(dist)]))
        samples.append(sample)
    return samples

def to_device(batch, device):
    feed_dict = copy.deepcopy(batch)
    node_1, node_2, node_2_neg, graphs,total_graphs,item_graphs = feed_dict.values()
    # to device
    feed_dict["node_1"] = [x.to(device) for x in node_1]
    feed_dict["node_2"] = [x.to(device) for x in node_2]
    feed_dict["node_2_neg"] = [x.to(device) for x in node_2_neg]
    feed_dict["graphs"] = [g.to(device) for g in graphs]
    feed_dict["total_graphs"] = [g.to(device) for g in total_graphs]
    feed_dict["item_graphs"] = [g.to(device) for g in item_graphs]
