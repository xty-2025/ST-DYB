...
class GATConv(nn.Module):
    ...
    def forward(self, graph, feat):
        with graph.local_scope():
            #h_src is source node of edge
            h_src = h_dst = self.feat_drop(feat).view(-1, self._num_heads, self.in_size)

            el = (h_src * self.attn_l).sum(dim=-1).unsqueeze(-1)#maybe calculate the left node feature of the edge
            er = (h_dst * self.attn_r).sum(dim=-1).unsqueeze(-1)#maybe calculate the right node feature of the edge
            #update src node feature
            ...
            # message passing
            graph.update_all(fn.u_mul_e('ft', 'a', 'm'), fn.sum('m', 'ft'))#'m' is multi output
            ...
            #rst is result of target node feature
            return rst
    ...
    def __init__(self, meta_paths, in_size, aggre_type, layer_num_heads, dropout):
        ...
        for i in range(len(meta_paths)):
            ...
            if meta_paths[i][0][0] == meta_paths[i][-1][-1]:
                ...
                #hete graph
                self.gat_layers.append(HetGCNLayer(in_size, aggre_type, self.num_heads,
                                                   dropout, dropout, activation=F.elu, residual=False))
...
class HMSG(nn.Module):
    def __init__(self, meta_paths, in_size, hidden_size, aggre_type, num_heads, dropout):
        ...
        self.fc_u = nn.Linear(in_size['user'], hidden_size*num_heads) # in_features=64, out_features= 8 * 8
        self.fc_i = nn.Linear(in_size['item'], hidden_size*num_heads) # in_features=64, out_features= 8 * 8

        self.hidden_size = hidden_size
        self.num_heads = num_heads
        self.layers1 = HMSGLayer(meta_paths, hidden_size, aggre_type, num_heads, dropout) #meta_path = [['ui', 'iu'], ['iu', 'ui'], ['ui'], ['iu']] hidden_size = 8 aggre_type = mean
        ...
        self.fc = nn.Linear(hidden_size*num_heads, hidden_size*num_heads)
    ...
    def forward(self, group):
        ...
        inputs = group[1]
        h_trans = {}
        h_trans['user'] = self.fc_u(inputs['user_feat']).view(-1, self.num_heads, self.hidden_size)
        h_trans['item'] = self.fc_i(inputs['item_feat']).view(-1, self.num_heads, self.hidden_size)
