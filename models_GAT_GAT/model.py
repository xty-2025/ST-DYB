...
class LPreplus(nn.Module):
    ...
    def forward(self, args, user_graphs,total_graphs,item_graphs):
        ...
        mono_item_out = []
        for t in range(0, self.num_time_steps):
            mono_user_out.append(self.tanh(self.mono_weight_u[t] * GCN_out_u[t] + (1 - self.mono_weight_u[t]) * structural_user_out[t].x))
            mono_item_out.append(self.tanh(self.mono_weight_i[t] * GCN_out_i[t] + (1 - self.mono_weight_i[t]) * structural_item_out[t].x))
        ...
        #user
        temporal_user_out = self.temporal_attn(structural_outputs_user_padded)
        #item
        temporal_item_out = self.temporal_attn(structural_outputs_item_padded)
        out = torch.cat([temporal_user_out,temporal_item_out],dim=0)# 800*16*32
        temporal_att_out = torch.transpose(out, 0, 1)

        structural_outputs = torch.cat([structural_outputs_user_padded, structural_outputs_item_padded], dim=0)#800*16*16
        ...
        return final_out_cz, final_out_sz
    ...
    def evaluate(self, train_edges_pos, train_edges_neg, val_edges_pos, val_edges_neg, test_edges_pos, test_edges_neg, emb):
