...
def evaluate_classifier(train_pos, train_neg, val_pos, val_neg, test_pos, test_neg, source_embeds, target_embeds):
    ...
    val_results_auc=defaultdict(lambda: [])
    test_results_auc=defaultdict(lambda: [])
    val_results_micro=defaultdict(lambda: [])
    test_results_micro=defaultdict(lambda: [])
    ...
    val_results_auc['SIGMOID'].extend([val_auc, val_auc])
    test_results_auc['SIGMOID'].extend([test_auc, test_auc])
    val_results_micro['SIGMOID'].extend([val_auc, val_auc])
    test_results_micro['SIGMOID'].extend([test_auc, test_auc])
    ...
    for operator in operatorTypes:
        ...
        #val_roc_score_auc= roc_auc_score(val_labels, val_predict)
        #test_roc_score_auc= roc_auc_score(test_labels, test_predict)
        fpr,tpr,thresholds=roc_curve(test_labels, test_predict)
        auc_result = auc(fpr,tpr)
        test_results_score_micro=auc_result
        fpr,tpr,thresholds=roc_curve(val_labels, val_predict)
        auc_result=auc(fpr,tpr)
        val_results_score_micro=auc_result         

        precision, recall, threshold = precision_recall_curve(test_labels, test_predict)
        test_roc_score_auc = auc(recall, precision)
        precision, recall, threshold = precision_recall_curve(val_labels, val_predict)
        val_roc_score_auc = auc(recall, precision)


        val_results[operator].extend([val_roc_score, val_roc_score])
        test_results[operator].extend([test_roc_score, test_roc_score])
        val_results_auc[operator].extend([val_roc_score_auc, val_roc_score_auc])
        test_results_auc[operator].extend([test_roc_score_auc, test_roc_score_auc])
        val_results_micro[operator].extend([val_results_score_micro, val_results_score_micro])
        test_results_micro[operator].extend([test_results_score_micro, test_results_score_micro])

        val_pred_true[operator].extend(zip(val_predict, val_labels))
        test_pred_true[operator].extend(zip(test_predict, test_labels))


    return val_results, test_results, val_pred_true, test_pred_true,val_results_auc,test_results_auc,val_results_micro,test_results_micro
