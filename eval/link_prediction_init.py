...
def get_roc_score_t(edges_pos, edges_neg, source_emb, target_emb):
    ...
    adj_rec = np.dot(source_emb, target_emb.T)  # 两两节点的关系 厨师节点 目标节点
    pred = []
    pos = []
    for e in edges_pos:
    ...
    labels_all = np.hstack([np.ones(len(pred)), np.zeros(len(pred_neg))])#正样本为1 负样本为0
    roc_score = roc_auc_score(labels_all, pred_all)#计算精度
    return roc_score
