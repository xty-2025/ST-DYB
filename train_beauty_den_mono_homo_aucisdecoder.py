...
if __name__ == "__main__":
    ...
    parser.add_argument('--graph_name', type=str, nargs='?',
                        default='../dynhen+HMSG+fourier_415/data/beauty/graphs.pkl',
                        help='user graph name')
    parser.add_argument('--item_graph_name', type=str, nargs='?',
                        default='../dynhen+HMSG+fourier_415/data/beauty/item_graphs.pkl',
                        help='item graph name')
    parser.add_argument('--total_graph_name', type=str, nargs='?',
                        default='../dynhen+HMSG+fourier_415/data/beauty/total_graphs.pkl',
    ...
    parser.add_argument('--useradj_name', type=str, nargs='?',
                        default='../dynhen+HMSG+fourier_415/data/beauty/user_adj.pkl',
                        help='useradj name')
    parser.add_argument('--itemadj_name', type=str, nargs='?',
                        default='../dynhen+HMSG+fourier_415/data/beauty/item_adj.pkl',
                        help='itemadj name')
    parser.add_argument('--context_pairs_path', type=str, nargs='?',
                        default='../dynhen+HMSG+fourier_415/data/beauty/context_pairs.pkl',
                        help='result of randomwalk')
    parser.add_argument('--datapath', type=str, nargs='?', default='../dynhen+HMSG+fourier_415/data/beauty',
                        help='datapath')
    ...
    parser.add_argument('--structural_head_config', type=str, nargs='?', default='32, 16, 8',
                        help='Encoder layer config: # attention heads in each GAT layer')
    parser.add_argument('--structural_layer_config', type=str, nargs='?', default='128, 16',
    ...
    parser.add_argument('--backbone', type=str, default='dida')
    ...
    parser.add_argument('--spec_res', type=int, default=0, help='')
    ...
    parser.add_argument('--self_att_inf_alpha', type=float, nargs='?', default=0.2)
    parser.add_argument('--rnn_units', type=int, nargs='?', default=8,
                        help='gru units num')
    parser.add_argument('--output_dim', type=int, nargs='?', default=1,
                        help='model output dim is 800 * output_dim')
   ...
   # item_graphs
    ...
    # randomwalk get context
    # pos-sample neg-sample
    ...
    train_edges_pos = torch.transpose(torch.from_numpy(train_edges_pos), 0, 1)
    train_edges_neg = torch.transpose(torch.tensor(train_edges_neg), 0, 1)
    val_edges_pos = torch.transpose(torch.from_numpy(val_edges_pos), 0, 1)
    val_edges_neg = torch.transpose(torch.tensor(val_edges_neg), 0, 1)
    test_edges_pos = torch.transpose(torch.from_numpy(test_edges_pos), 0, 1)
    test_edges_neg = torch.transpose(torch.tensor(test_edges_neg), 0, 1)
    ...
    model = LPreplus(args, feats[0].shape[1], args.time_steps).to(device)
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        _, _, _, emb_cz, _ = model.get_final_emb(args, feed_dict)
        emb_cz = torch.transpose(emb_cz, 0, 1)
        emb = emb_cz[:, -2, :].detach().cpu()#should be 800*16

        _, _, val_auc, val_ap, test_auc, test_ap= model.evaluate(train_edges_pos,
                                                              train_edges_neg,
                                                              val_edges_pos,
                                                              val_edges_neg,
                                                              test_edges_pos,
                                                              test_edges_neg,
                                                              emb)
        epoch_auc_val = val_auc
        epoch_auc_test = test_auc


        if epoch_auc_val > best_epoch_val:
            best_epoch_val = epoch_auc_val
            torch.save(model.state_dict(), "./model_checkpoints/model_mono_homo.pt")
            patient = 0
        else:
            patient += 1
        ...
        print("Epoch {:<3},  Loss = {:.3f}, Val AUC {:.3f} Test AUC {:.3f}".format(epoch,
                                                                                   ...
                                                                                   epoch_auc_test))
        # Test Best Model
    model.load_state_dict(torch.load("./model_checkpoints/model_mono_homo.pt"))
    model.eval()

    _, _, _, emb_cz, _ = model.get_final_emb(args, feed_dict)
    emb = emb_cz[:, -2, :].detach().cpu().numpy()
