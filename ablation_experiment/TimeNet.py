...
from models_GAT_GAT.utils import SpaSeqNetLast, seq2str
from models_GAT_GAT.misc import DummyArgs
from torch.fft import rfft, irfft, fft, ifft
from ablation_experiment.dida import DGNN


from sklearn.metrics import f1_score, accuracy_score
from ablation_experiment.misc import EarlyStopping, move_to, cal_metric
# from ablation_experiment.utils import COLLECTOR
from libwon.utils.collector import *
from tqdm import tqdm, trange
from ablation_experiment.utils import *
...
class TimeNet(torch.nn.Module):
    def __init__(self, args):
        ...
        self.args = args
        self.linear = nn.Linear(16, hid_dim, bias=False)
    ...
    def forward(self, graph_feature):
        ...
        H_cz, H_sz = self.get_final_emb(feature)#16 800 16
        return H_cz, H_sz
    ...
    def spectral_filter(self, z):
        ...
        #irfft
        # zc_recon = irfft(c_spec, n=self.spec_len, norm="ortho", dim=1)
        # zs_recon = irfft(s_spec, n=self.spec_len, norm="ortho", dim=1)
        # h_rec = 0.9 * zc_recon + 0.1 * zs_recon
        # out = h_rec
        #return out
    ...
    def get_final_emb(self, feature):
        ...
        for t in range(self.time_length):
            ...
            # cs.append(cz)
            # ss.append(sz)
            cz, sz = self.spectral_filter(x_list)  # [N, d] is node_num * nhid(16) 800 16
            H_cz.append(cz)
            H_sz.append(sz)

        # cs = torch.stack(cs, dim=0)  # [T, N, d]
        # ss = torch.stack(ss, dim=0)  # [T, N, d]
        H_cz = torch.stack(H_cz, dim=0)# [T, N, d] 16 800 16
        H_sz = torch.stack(H_sz, dim=0)
        return H_cz, H_sz
