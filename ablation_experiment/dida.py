...
from ablation_experiment.utils import MultiplyPredictor, LinkPredictor
...
class DGNNLayer(nn.Module):
    ...
    def __init__(self, in_dim, hid_dim, n_heads, norm=True, dropout=0, skip=False, use_RTE=False, sample_r=0.1, use_fmask=False, only_causal=False):
        ...
        if norm == 0:
            ...
            self.update_norm = nn.BatchNorm1d(hid_dim)
    ...
    def collect_neighbors(self, edge_index_list, x_list): # edge_index_list is the edge sets, x_list is the feature list
        """
        for each time t : collect edges of 1:t for aggregation
        return [x_tar,topos]
        """
        twin = len(x_list)#time len
...
class DGNN(nn.Module):
    ...
    def __init__(self, args=None):
        ...
        self.feat = Parameter((torch.ones(args.total_node, args.nfeat)).to(args.device), requires_grad=True)
    ...
    def forward(self, edge_index_list, x_list):
        ...
        if x_list is None:
            x_list = [self.linear(self.feat) for i in range(time_len)]
        else:
            x_list = [self.linear(x) for x in x_list[:time_len]]
