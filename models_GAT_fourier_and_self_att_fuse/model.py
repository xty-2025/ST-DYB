...
from models_GAT_fourier_and_self_att_fuse.layers import StructuralAttentionLayer, TemporalAttentionLayer, GCNLayer, InteractionModule
from models_GAT_fourier_and_self_att_fuse.TimeNet import TimeNet
...
class fourier_and_self_att_fuse(nn.Module):
    def __init__(self, args, num_features, time_length):
        ...
        """
        super(fourier_and_self_att_fuse, self).__init__()
        self.args = args
        if args.window < 0:
    ...
    def forward(self, args, user_graphs,total_graphs,item_graphs):
        ...
        temporal_item_self_att_out = torch.transpose(temporal_item_out, 0, 1)
        ...
        #self_att + fourier
        # H_user = self.fourier_weight * temporal_user_self_att_out + (1-self.fourier_weight) * temp_user #16 190 16 timestep*node_num_dim
        # H_item = self.fourier_weight * temporal_item_self_att_out + (1-self.fourier_weight) * temp_item   # 16 610 16 timestep*node_num*dim
        ...
        final_user = 0.8 * structural_user_out + 0.8 * self.interaction(temporal_user_self_att_out, temp_user)
        final_item = 0.8 * structural_item_out + 0.8 * self.interaction(temporal_item_self_att_out, temp_item)

        H = torch.cat([final_user, final_item],dim=1)#16*800*16
