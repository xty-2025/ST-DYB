...
if __name__ == "__main__":
    ...
    parser.add_argument('--total_node', type=int, nargs='?', default=11334,
                        help="total node")
    parser.add_argument('--user_num', type=int, nargs='?', default=1752,
                        help="user_num")
    parser.add_argument('--user_graph_name', type=str, nargs='?',
                        default='/root/DySAT_pytorch10_27copy/data/movie/user_graphs.pkl',
                        help='user_graph name')
    parser.add_argument('--item_graph_name', type=str, nargs='?',
                        default='/root/DySAT_pytorch10_27copy/data/movie/item_graphs.pkl',
                        help='item_graph name')
    parser.add_argument('--total_graph_name', type=str, nargs='?',
                        default='/root/DySAT_pytorch10_27copy/data/movie/total_graphs.pkl',
    ...
    parser.add_argument('--useradj_name', type=str, nargs='?',
                        default='/root/DySAT_pytorch10_27copy/data/movie/user_adj.pkl',
                        help='useradj name')
    parser.add_argument('--itemadj_name', type=str, nargs='?',
                        default='/root/DySAT_pytorch10_27copy/data/movie/item_adj.pkl',
                        help='item_adj name')
    parser.add_argument('--context_pairs_path', type=str, nargs='?',
                        default='/root/DySAT_pytorch10_27copy/data/movie/context_pairs.pkl',
                        help='result of randomwalk')
    parser.add_argument('--datapath', type=str, nargs='?', default='/root/DySAT_pytorch10_27copy/data/movie',
                        help='datapath')
    ...
    with open(args.useradj_name, 'rb') as f:
        user_adjs = pkl.load(f)
    with open(args.itemadj_name, 'rb') as f:
        item_adjs = pkl.load(f)
    with open((args.user_graph_name), "rb") as f:
        user_graphs = pkl.load(f)
    with open((args.user_graph_name),'rb') as f:
        item_graphs= pkl.load(f)
    with open((args.total_graph_name), "rb") as f:
        total_graphs = pkl.load(f)
    user_adjs=user_adjs[0:args.time_steps]
    item_adjs=item_adjs[0:args.time_steps]
    user_graphs=user_graphs[0:args.time_steps]
    item_graphs=item_graphs[0:args.time_steps]
    total_graphs=total_graphs[0:args.time_steps]
    # total adjs
    total_adjs_arr = load_adj(args.datapath)
    total_adjs_arr=total_adjs_arr[0:args.time_steps]
    ...
    assert args.time_steps <= len(user_adjs), "Time steps is illegal"
    # node2vec
    ...
    context_pairs_train=context_pairs_train[0:args.time_steps]
    ...
    dataset = MyDataset(args, total_graphs, user_graphs, feats, item_adjs, user_adjs, total_adjs_arr, context_pairs_train)
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        item_feat_list = []
        #
        out_put= model(args, feed_dict["user_graphs"], feed_dict["total_graphs"],feed_dict["item_graphs"])[:, -2, :].detach().cpu().numpy()
        print(type(out_put))
        user_emb = out_put[0:args.user_num]
        item_emb = out_put[args.user_num:args.total_node]
        print(type(user_emb),'123456787654321',type(item_emb))
        user_emb_list.extend(user_emb)
        item_feat_list.extend(item_emb)
        emb = user_emb_list + item_feat_list
        emb = np.array(emb)
        print(type(emb))
        print(emb.shape)
        val_results, test_results, _, _ ,val_results_auc,test_results_auc,_,_= evaluate_classifier(train_edges_pos,
                                                              train_edges_neg,
                                                              val_edges_pos,
                                                              val_edges_neg,
                                                              test_edges_pos,
        ...
        if epoch_auc_val > best_epoch_val:
            ...
            if patient > args.early_stop:
                break
        print("Epoch {:<3},  Loss = {:.3f}, Val AUC {:.3f} Test AUC {:.3f} Val auc_pr {:.3f} Test auc_pr {:.3f}".format(epoch,
                                                                                   np.mean(epoch_loss),
                                                                                   epoch_auc_val,
                                                                                   epoch_auc_test,
                                                                                   epoch_auc_val_auc,
                                                                                   epoch_auc_test_auc))
        if(epoch_auc_test>best_test):
            best_test=epoch_auc_test
        print("time_steps{:<3},best_test is {:.3f}".format(args.time_steps,best_test))
        # Test Best Model
    model.load_state_dict(torch.load("./model_checkpoints/model_small_dataset.pt"))
    model.eval()

    emb = model(args, feed_dict["user_graphs"], feed_dict["total_graphs"],feed_dict["item_graphs"])[:, -2, :].detach().cpu().numpy()  # 190*16*128
    ...
    auc_val_mic = val_results_auc["HAD"][1]
    auc_test_mic = test_results_auc["HAD"][1]
