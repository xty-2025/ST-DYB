...
class DCGRUCell(torch.nn.Module):
    ...
    def forward(self, adj, inputs, hx):
        ...
        c = self._gconv(inputs, adj, r * hx, self._num_units)
    ...
    def _gconv(self, inputs, adj, state, output_size, bias_start=0.0):
        ...
        if self._max_diffusion_step == 0:
            ...
            x1 = torch.sparse.mm(adj_coo, x0)
            x = self._concat(x, x1)

            for k in range(2, self._max_diffusion_step + 1):
                x2 = 2 * torch.sparse.mm(adj_coo, x1) - x0
                x = self._concat(x, x2)
                x1, x0 = x2, x1
