...
from models_GAT_fourier_and_diffusion.dcrnn_model import DCRNNModel
from models_GAT_fourier_and_diffusion.layers import StructuralAttentionLayer, TemporalAttentionLayer, GCNLayer
from models_GAT_fourier_and_diffusion.TimeNet import TimeNet
from utils.utilities import fixed_unigram_candidate_sampler
device = torch.device("cpu" if torch.cuda.is_available() else "cpu")

class fourier_and_diffusion(nn.Module):
    def __init__(self, args, num_features, time_length):
        ...
        """
        super(fourier_and_diffusion, self).__init__()
        self.args = args
        if args.window < 0:
        ...
        self.fourier_weight = nn.Parameter(torch.rand(1, requires_grad=True))
        self.diff_output_dim = self.args.output_dim * self.args.total_node
        ...
        self.GCN, self.structural_attn, self.temporal_attn, self.time_emb, self.dcrnn_model = self.build_model()
        self.bceloss = BCEWithLogitsLoss()
        self.linear = nn.Linear(self.args.total_node * self.args.output_dim, self.temporal_layer_config[-1], bias=False)
    ...
    def forward(self, args, user_graphs, total_graphs, item_graphs):
        ...
        for t in range(0,self.num_time_steps):
            dcrnn_input_x.append(total_graphs[t].x)
            dcrnn_input_adj.append(total_graphs[t].adj)

        dcrnn_output = self.dcrnn_model(dcrnn_input_adj, dcrnn_input_x)
        dcrnn_output_list = [self.linear(x) for x in dcrnn_output]
        dcrnn_output = torch.stack(dcrnn_output_list)
        dcrnn_output = torch.transpose(dcrnn_output, 0, 1)
        print(dcrnn_output.shape)
        
        # dcrnn_output_u = []
        # dcrnn_output_i = []
        # for t in  range(0,self.num_time_steps):
        #     dcrnn_output_u.append(dcrnn_output[t][0:user_num])
        #     dcrnn_output_i.append(dcrnn_output[t][user_num:len(dcrnn_output[t])])
        # dcrnn_output_u = torch.stack(dcrnn_output_u)
        # dcrnn_output_i = torch.stack(dcrnn_output_i)
        ...
        for t in range(0, self.num_time_steps):
            user_group=[]
            user_group.append(dcrnn_output_u[t])
            user_group.append(user_graphs[t])
            item_group=[]
            item_group.append(dcrnn_output_i[t])
        ...
        temporal_user_self_att_out = torch.transpose(temporal_user_out, 0, 1)
        temporal_item_self_att_out = torch.transpose(temporal_item_out, 0, 1)#16 *610 *16
        ...
        # print("EEEEEEEEEEEE")
        # print(temporal_user_out.shape)#16*800*16
        # input(ddd)'''

        # temp_user = self.time_emb(dcrnn_output_u)
        # temp_item = self.time_emb(dcrnn_output_i)
        
        
        '''

        #gat+self_att+fourier

        structural_user_out = torch.transpose(structural_outputs_user_padded, 0, 1)  # timestep*user_num*dim 16 190 32
        structural_item_out = torch.transpose(structural_outputs_item_padded, 0, 1)  # timestep*item_num*dim 16 190 32


        H_user =   0.9 * structural_user_out +  args.self_att_inf_alpha * temporal_user_self_att_out + args.fourier_inf_alpha * temp_user  # 16 190 16 timestep*node_num_dim
        H_item =   0.9 * structural_item_out +  args.self_att_inf_alpha * temporal_item_self_att_out + args.fourier_inf_alpha * temp_item   # 16 610 16 timestep*node_num*dim

        H = torch.cat([H_user,H_item],dim=1)
        H = torch.transpose(H, 0, 1) #
        '''
        # H = torch.cat([temp_user,temp_item],dim=1)
        # H = torch.transpose(H, 0, 1)
        return dcrnn_output


    def build_model(self):
        dcrnn_model = DCRNNModel(self.args)
        ...
        input_dim=self.diff_output_dim
        ...
        return GCN_layers, structural_attention_layers, temporal_attention_layers, Time_emb_layers, dcrnn_model
