...
from ablation_experiment.model import LPreplus
...
if __name__ == "__main__":
    ...
    parser.add_argument('--weight_decay', type=float, nargs='?', default=0.0005,
                        help='Initial learning rate for self-attention model.')
    # Architecture params
    parser.add_argument('--GCN_layer_config', type=str, nargs='?', default='16',
   ...
   # item_graphs
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        emb = emb_cz[:, -2, :].detach().cpu().numpy()#should be 800*16

        val_results, test_results, _, _ ,_, _, _, _= evaluate_classifier(train_edges_pos,
                                                              train_edges_neg,
                                                              val_edges_pos,
                                                              val_edges_neg,
                                                              test_edges_pos,
        ...
        print("Epoch {:<3},  Loss = {:.3f}, Val AUC {:.3f} Test AUC {:.3f} ".format(epoch, np.mean(epoch_loss),
                                                                                    epoch_auc_val, epoch_auc_test))
        # Test Best Model
